...
export async function fetchRepoDataViaGit(owner, repo, token, onProgress, { existing } = {}) {
  ...
  try {
    ...
    // Stream git log with unified diff. We do NOT use pathspecs here because
    // that would drop commits whose only changes were binary/vendored files,
    // breaking the timeline. Instead the parser skips binary-extension files
    // in-place (diff data flows out of git but isn't retained in memory).
    const fmt = `${DELIM_PREFIX}%n%H%n%ct%n%an%n%s`;
    const logArgs = [
      '-c', 'diff.renameLimit=2000',
      'log', '--reverse', '-M', '-p', `--format=format:${fmt}`, 'HEAD',
    ];
