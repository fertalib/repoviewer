...
/**
 * Fetch file sizes from GitHub Trees API. Returns Map<path, estimatedLines>.
 * Uses blob size / 30 as line estimate (avg ~30 bytes/line for source code).
 */
async function fetchFileSizeMap(owner, repo, branch, token) {
  const tree = await gh(`/repos/${owner}/${repo}/git/trees/${branch}?recursive=1`, token);
  const map = new Map();
  for (const entry of tree.tree) {
    if (entry.type !== 'blob') continue;
    if (BINARY_EXT_RE.test(entry.path)) continue;
    map.set(entry.path, Math.max(1, Math.round((entry.size || 30) / 30)));
  }
  return map;
}
  ...
  for (const commit of commits) {
    for (const file of commit.files) {
      if (file.status === 'added') {
        const est = fileSizeMap.get(file.filename) || 10;
        file.additions = est;
        knownSizes.set(file.filename, est);
      } else if (file.status === 'removed') {
        file.deletions = knownSizes.get(file.filename) || fileSizeMap.get(file.filename) || 10;
        knownSizes.delete(file.filename);
      } else if (file.status === 'renamed') {
        const old = knownSizes.get(file.previous_filename) || fileSizeMap.get(file.filename) || 10;
...
export async function fetchRepoDataViaGit(owner, repo, token, onProgress, { existing } = {}) {
  ...
  try {
    ...
    // Fetch file sizes from GitHub Trees API (single REST call) to estimate
    // per-file line counts. Then enrich commits with those estimates.
    log({ stage: 'tree', message: 'Estimating file sizes...' });
    const fileSizeMap = await fetchFileSizeMap(owner, repo, branch, token);
