...
// Cap on PRs fetched. GitHub's /pulls response includes huge `body` markdown
// fields — 100 PRs per page can easily be tens of MB. Without a cap, repos
// with thousands of PRs OOM the JSON.parse in `res.json()`.
const MAX_PR_PAGES = Number(process.env.REPOVIEWER_MAX_PR_PAGES) || 5; // 500 PRs max
...
async function fetchPRs(owner, repo, token, log) {
  ...
  for (let page = 1; page <= MAX_PR_PAGES; page++) {
    log({ stage: 'prs', message: `Fetching merged PRs (page ${page})...` });
    let data;
    try {
      data = await gh(`/repos/${owner}/${repo}/pulls?state=closed&sort=updated&direction=desc&per_page=100&page=${page}`, token);
    } catch (err) {
      console.warn(`[${owner}/${repo}] PR fetch page ${page} failed: ${err.message}`);
      break;
    }
    if (!Array.isArray(data) || data.length === 0) break;
    // Slim immediately — drop body/head/base/user refs so GC can free the
    // raw objects. Only keep what the viz uses.
  ...
  slim.sort((a, b) => new Date(a.mergedAt) - new Date(b.mergedAt));
  return slim;
}
